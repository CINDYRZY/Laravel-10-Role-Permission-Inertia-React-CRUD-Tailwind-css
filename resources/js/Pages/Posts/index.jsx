...
export default function Dashboard(props) {
    ...
    return (
        <AuthenticatedLayout auth={props.auth} user={props.auth.user} errors={props.errors} header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">All Posts</h2>}>
            <Head title="Posts" />
            <div className="bg-purple overflow-hidden shadow-sm sm:rounded-lg">
                {props.flash.success && (
                    <div className="text-center bg-purple-300 font-bold text-blue-900 p-1">
                        {props.flash.success}
                    </div>
                )}
            </div>
            <div className="py-12">
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <div className="p-6 bg-blue border-b border-purple-200">
                            <div className="flex items-center justify-between mb-6">
                                <Link className="flex items-center px-6 py-2 text-white bg-purple-500 rounded-md focus:outline-none" href={route("posts.create")}>
                                    ...
                                    {posts && posts.map((item) => (
                                        <tr key={item.id}>
                                            ...
                                            <td className="border px-4 text-center py-2">
                                                <Link tabIndex="1" className="px-4 py-2 text-sm text-white bg-blue-500 rounded" href={route("posts.edit", item.id)} >Edit</Link>
